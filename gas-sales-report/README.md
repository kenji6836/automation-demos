...
### Google スプレッドシート上の実行結果（`previewReport` 実行後の「月次レポート」シート）

![「月次レポート」シート（Google スプレッドシート上で previewReport を実行した結果）](docs/report-sheet.jpg)

![「売上データ」シート（sales.csv 200 行）→「月次レポート」シートへの切り替え](docs/sheet-demo.gif)

同梱の `sales.csv`（サンプル 200 行・2026 年 3〜8 月）を「売上データ」シートに貼り付け、Apps Script で `previewReport` を実行したときの実画面です（2026-09-14 撮影）。
月の表記や `+4.2%` などの前月比は文字列のまま書き込まれ、金額は 3 桁区切りで表示されます。

> Slack 投稿の画面は Incoming Webhook を用意していないため未掲載です（`SLACK_WEBHOOK_URL` を設定すると下のペイロードがそのまま投稿されます）。

### ローカル実行の実出力

同じ `sales.csv` を `node scripts/run-local.js` で集計した出力です。GAS 上ではこの表がそのまま「月次レポート」シートに `setValues()` され、下のメール本文と Slack ペイロードが送信されます。
