...
### 実出力例（建設/工務店プロファイル・同梱 5 通・モデル `claude-haiku-4-5`・2026-09-12 実行）

`drafts/kensetsu/` にそのまま入っています（`*.md` が人が読む用・`*.json` が構造化データ）。5 通で API 費用 $0.0216（≈¥3）。

| 問い合わせ | 分類 | 緊急度 | 要約 1 行目 |
|---|---|---|---|
| `01_reform_estimate` | 見積依頼 | 中 | タイル張りの浴室（約1坪）をユニットバスへリフォームしたいとのご相談です。 |
| `02_gaikou_estimate` | 見積依頼 | 中 | 砂利敷き駐車場（2台分）をコンクリート舗装、道路側に約8mの目隠しフェンス設 |
| `03_shinchiku_soudan` | 見積依頼 | 中 | 架空市はるか町の約50坪の土地で、3人家族向け2階建て新築を検討中。 |
| `04_koji_renraku` | 施工中の連絡 | 高 | 明日9月13日の施工工程と騒音が出る作業の時間帯を確認したい |
| `05_shizai_eigyo` | その他 | 低 | 資材メーカーから住宅用断熱材の新商品カタログの案内・取引先募集 |

見積依頼 3 通の返信下書きはいずれも確認事項 5 点（施工場所・希望時期・予算・図面/写真・連絡時間帯）を箇条書きで尋ね、金額・工期は断定していません。「概算は現地確認後」の明示は 01・02 の 2 通で、03（新築相談）は「正確な概算見積りのため」の情報依頼に留まりました＝プロンプトの指示が 1 通で抜けた例として残しています（`drafts/kensetsu/0*.md` を参照）。
